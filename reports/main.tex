...
\begin{abstract}
This document presents the dataset charter and the first processed version for the sports performance analysis project. The objective is to map the tactical evolution and results of the UEFA Champions League between 2011 and 2025 by integrating data from Open Football with advanced statistics from the ESPN API. The project now leverages \textbf{Parquet} for high-performance data handling and implements \textbf{Principal Component Analysis (PCA)} to reduce 33 tactical features into latent embeddings, identifying 19 components that explain 90\% of the statistical variance across 4,065 player-season profiles.
\end{abstract}
...
"How have possession patterns and shooting effectiveness evolved in the Champions League across decades, and can we identify distinct tactical 'archetypes' by reducing the dimensionality of 4,000+ player-season performance profiles?"
...
\begin{table}[h!]
    ...
    \begin{tabular}{ll}
        ...
        Original Format& JSON / API Response / Parquet\\
        Estimated Size& ~12 MB (Processed V1 - Parquet \& CSV)\\
        \bottomrule
    \end{tabular}
...
\end{figure}
    ...
    \item \textbf{Feature Engineering:} We integrated the \textbf{UEFA Official API} for match officials and \textbf{ESPN Summary API} for statistical events. Furthermore, categorical variables (position, age groups) were encoded for PCA.
    ...
    \item \textbf{Imputation:} Statistical fields were fully backfilled using API discovery. The project migrated to \textbf{Parquet} storage to preserve schema integrity (e.g., Int64/Float64 types) during high-dimensional analysis.
\end{itemize}
...
# 1. Install dependencies (requires pyarrow)
pip install -r requirements.txt

# 2. Execute automated ingestion and enrichment pipeline
python src/main.py

# 3. Build PCA feature matrix from Parquet stats
...
\begin{itemize}
    ...
    \item \textbf{Dimensionality Reduction}: The PCA pipeline identifies that \textbf{19 components} are required to capture 90\% of the variance in the 33-feature space. The first two components (PC1 and PC2) account for approximately \textbf{28.07\%} of the variance, providing a clear visualization of tactical clusters.
\end{itemize}
